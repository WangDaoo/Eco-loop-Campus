...
export function mapSubmissionRow(row: Row): RecyclingSubmission {
  return {
    ...
    correctedClass: text(row.correctedClass ?? row.corrected_class) || undefined,
    correctedWasteTypeId: text(row.correctedWasteTypeId ?? row.corrected_waste_type_id) || undefined,
...
export function toSubmissionRow(submission: RecyclingSubmission): Row {
  return {
    ...
    prediction_id: submission.predictionId ?? null,
    corrected_class: submission.correctedClass ?? null,
    corrected_waste_type_id: submission.correctedWasteTypeId ?? null,
