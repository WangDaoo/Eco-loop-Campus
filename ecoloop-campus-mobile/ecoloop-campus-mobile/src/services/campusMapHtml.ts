...
function getPosition(station: BinStation, index: number) {
  const base = fixedPositions[station.building] || fallbackPositions[index % fallbackPositions.length];
  return {
    x: typeof station.mapX === 'number' ? station.mapX : base.x,
    y: typeof station.mapY === 'number' ? station.mapY : base.y,
  };
}
...
export function buildCampusMapHtml(stations: BinStation[]): string {
...
.eg-pin:hover,.eg-pin:focus{transform:scale(1.15)}
.eg-pin-inner{width:10px;height:10px;border-radius:50%;background:rgba(255,255,255,.85)}
.leaflet-tooltip{
...
<body>
<div id="map"></div>
<button id="recenter" onclick="recenter()">⊙ Căn giữa campus</button>
...
stations.forEach(function(s){
  ...
  var icon=L.divIcon({
    className:"",
    html:'<div class="eg-pin" style="background:'+s.color+'"><div class="eg-pin-inner"></div></div>',
