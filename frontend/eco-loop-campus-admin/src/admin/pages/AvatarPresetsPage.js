...
export default function AvatarPresetsPage() {
  ...
  const editPreset = item => setForm({ ...initialForm, ...item });
  ...
  const handleFile = async event => {
    ...
    if (!response.data?.imageUrl) {
      showToast("Chưa upload được ảnh avatar lên máy chủ", "danger");
      return;
    }
  ...
  const columns = [
    ...
    { key: "label", label: "Tên", render: row => <div><strong>{row.label}</strong><span className="eg-muted-block">{row.key}</span></div> },
    { key: "imageUrl", label: "Nguồn ảnh", render: row => row.imageUrl ? <a href={row.imageUrl} target="_blank" rel="noreferrer">Mở ảnh</a> : <span className="eg-muted-block">Dùng hình hoạt họa mặc định</span> },
    { key: "sortOrder", label: "Thứ tự" },
    { key: "status", label: "Trạng thái", render: row => <StatusBadge status={row.status} /> },
    { key: "actions", label: "Thao tác", render: row => <button type="button" className="eg-small-btn" onClick={() => editPreset(row)}>Sửa</button> },
  ];
    ...
    <div className="eg-page">
      ...
      <section className="eg-command-panel eg-avatar-command">
        <div>
          ...
          <p>Sinh viên sẽ chọn trong danh mục avatar đang hoạt động. Ảnh được admin upload và lưu trên máy chủ.</p>
        </div>
        <div className="eg-command-metrics">
          <article><span>Tổng preset</span><strong>{items.length}</strong></article>
          <article><span>Đang dùng</span><strong>{activeCount}</strong></article>
        </div>
      </section>

      <section className="eg-card">
        <div className="eg-card-head"><h2>{form.key ? "Sửa avatar" : "Thêm avatar"}</h2></div>
        <form className="eg-form eg-avatar-form" onSubmit={submitForm}>
          ...
          <label>
            Mã avatar
            <input value={form.key} onChange={event => updateForm({ key: event.target.value })} placeholder="mam-xanh" />
          </label>
          <label>
            Upload ảnh
          ...
          <label>
            Màu nền
            <input type="color" value={form.background} onChange={event => updateForm({ background: event.target.value })} />
          </label>
          <label>
            Màu khung
            <input type="color" value={form.tile} onChange={event => updateForm({ tile: event.target.value })} />
          </label>
          <label>
            Màu nhấn
          ...
          <label>
            Màu nét mặt
            <input type="color" value={form.face} onChange={event => updateForm({ face: event.target.value })} />
          </label>
