...
Assert-Contains $buildApkFromApiBat 'set /p "API_URL=Nhap API backend public: "' 'build_apk_from_api.bat must let the user enter a backend API URL interactively.'
Assert-Contains $buildApkFromApiBat 'EXPO_PUBLIC_API_URL' 'build_apk_from_api.bat must embed the backend API URL into the mobile release build.'
Assert-Contains $buildApkFromApiBat ':app:createBundleReleaseJsAndAssets --rerun-tasks --no-daemon' 'build_apk_from_api.bat must force the Expo release JS bundle to include the latest backend API URL.'
