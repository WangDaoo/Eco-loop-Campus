...
Assert-Contains $serverSetupBat '-WithDocker' 'setup_server_full.bat must ask the runtime script to install Docker Desktop automatically.'
...
Assert-Contains $serverSetupBat 'SETUP_BUILD_APK' 'setup_server_full.bat must allow server-only setup on laptops without Android Studio.'
Assert-Contains $serverSetupBat ':skip_apk_build' 'setup_server_full.bat must skip APK build when Android SDK is unavailable.'
Assert-Contains $serverSetupBat 'Android SDK' 'setup_server_full.bat must explain missing Android SDK instead of failing server setup.'
...
Assert-Contains $ensureScript "install', '--user', '--quiet', '--accept-license" 'ensure script must install Docker Desktop per-user without Android/server admin assumptions.'
